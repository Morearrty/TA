...
<head>
    ...
    <title>Pengajuan Kegiatan - TA-AMS</title>
        ...
        <div class="content-wrapper" id="content">
            ...
            <div class="top-bar">
                <div class="d-flex align-items-center">
                    ...
                    <h4 class="page-title mb-0 ms-md-2">Pengajuan Kegiatan</h4>
                </div>
            </div>
            ...
            <div class="card card-purple mb-4">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <div>
                        <i class="bi bi-file-earmark-text me-2 text-purple-600"></i>
                        <h5 class="header-purple mb-0 d-inline">Daftar Pengajuan Kegiatan - {{ $district->name }}</h5>
                    </div>
                    <a href="{{ route('district.proposals.create') }}" class="btn btn-purple">
                        <i class="bi bi-plus-circle"></i> Ajukan Kegiatan Baru
                    </a>
                </div>
                <div class="card-body">
                    @if(session('success'))
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped table-hover">
                            ...
                            <tbody>
                                @forelse($proposals as $proposal)
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            @else
                                                <span class="badge bg-danger badge-yellow">Rejected</span>
                                            @endif
                                        </td>
                                        <td>{{ $proposal->created_at->format('d M Y') }}</td>
                                        <td>
                                            <a href="{{ route('district.proposals.show', $proposal->id) }}" class="btn btn-purple btn-sm">
                                                <i class="bi bi-eye"></i> View
                                            </a>
                                            @if($proposal->status == 'pending')
                                                <a href="{{ route('district.proposals.edit', $proposal->id) }}" class="btn btn-yellow btn-sm">
                                                    <i class="bi bi-pencil"></i> Edit
                                                </a>
                                                <form action="{{ route('district.proposals.destroy', $proposal->id) }}" method="POST" class="d-inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete this proposal?')">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="8" class="text-center">No activity proposals found</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
